...
class calclass:
    ...
    def printFunction(mylist, cal, cnt):
        ...
        # 约分处理
        list_re =calclass.reductionFunction(mylist)
    ...
    def answerJudge():
        ...
        for i in open(r'D:\ruangong\answer.txt', encoding='utf-8'):
            ...
            list_an.append(an)
        cnt = len(list_an)
        list_an_calcutalor = calclass.calcutalorFileExercise()

        list_zuihou = []
        for line in list_an_calcutalor:
            list_yuan = calclass.reductionFunction(line)
        ...
        for i in range(1, cnt + 1):
            ...
            # 使用正则表达式来比较两个字符串的大小
            if calclass.compare_strings(str1, str2) == False:
                WCount += 1
                Wrong.append(i)
            else:
                CCount += 1
    ...
    def calcutalorFileExercise():
        ...
        for i in open(r'D:\ruangong\exercise.txt', encoding='utf-8'):
            ...
            del qu[-1]
            del qu[-1]
            answerList.append(calclass.calcuta(qu))
        return answerList
    ...
    def calcuta(list):
        ...
        for j in list_num:
            if j == '0':
                ...
                a, b = calclass.tofra(j)
                mylist[i].append(a)
                mylist[i].append(b)
            i += 1
        answer = calclass.calculatorFunction(mylist, list_cal)
        return answer
    ...
    def main():
        ...
        while cnt <= n:
            ...
            # -----计算部分-----
            # 计算答案1
            list_ans = calclass.calculatorFunction(mylist, cal)
            # 判断题目是否相同，如果一样将list_ans中的第二个数即分母乘以-1
            list_ans = calclass.judgeOnly(list_ans)
            # 答案约分
            ans_re = calclass.reductionFunction(list_ans)

            # 打印题目和答案
            if ans_re[0][0] != -1:
                calclass.printFunction(mylist, cal, cnt)
                calclass.printAnswer(ans_re, cnt)
                cnt = cnt + 1
